...
Every 10 minutes, `.github/workflows/update-coins.yml`:
...
This runs every 10 minutes using UTC. GitHub Actions schedules can lag under load; treat 10 minutes as a target, not a guarantee.

10 minutes was chosen to comfortably stay within CoinGecko's free "Demo" plan limits: 100 requests/minute and 10,000 call credits/month. At one request per run, this workflow makes ~144 requests/day — roughly 4,300–4,500/month — well under both caps, with plenty of headroom for manual `workflow_dispatch` runs or a shorter interval later if needed.
...
The frontend's own `fetch` already uses `cache: "no-cache"`, so on every page load the browser revalidates with the server anyway (a cheap `304` if nothing changed) rather than trusting its own cached copy. `max-age=60` instead bounds a *different* client's worst case: without it, a stale edge/proxy copy could combine with the up-to-10-minute gap between deploys to serve data up to ~20 minutes old; capping it at 60 seconds keeps the worst case closer to ~11 minutes.
